...
export default function Home() {
  ...
  const calcular = () => {
    const m = val(malha);
    const f = val(fio);
    const c = val(comprimento);
    const p = val(perda);
    const l = val(largura);
    const g = val(gancho);
    const pk = val(precoKg);
    const q = val(quantidade);

    if (m === 0 || f === 0 || c === 0 || l === 0 || pk === 0 || q === 0) {
      alert('Por favor, preencha todos os campos obrigatórios com valores numéricos válidos (Malha, Fio, Comprimento, Largura, Preço do Kg, Quantidade).');
      return;
    }
  ...
  const handleShareWhatsApp = async () => {
    ...
    const primeiraLinha = `${produto} - AB ${formatNumber(val(malha))} MM - FIO ${formatNumber(val(fio))} MM - ${comprimento} X ${largura} MM - ${gancho} MM${acabamento}`;
